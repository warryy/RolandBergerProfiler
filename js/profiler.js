var Profiler = function (config) {
  ...
  var drawHeatmap = function (data) {
    var canvas = $(container).find('canvas')[0],  // canvas
      ctx = canvas.getContext('2d'),  // 获取 ctx
      tempCanvas = document.createElement('canvas'),  // 定义一个临时的 canvas
      tempCtx = tempCanvas.getContext('2d'),  // 临时 canvas 的 ctx
      width = w,  // canvas 的宽高
      height = h,
      points = data;  // 格式化之后的 data
  ...
  var drawWord = function (data) {
    ...
    svg.append('rect').attr('fill', 'none').attr('stroke', 'black').attr('width', width).attr('height', height);
